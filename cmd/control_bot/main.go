...
func getToken() string {
	...
	return viper.GetString("token.control")
}
...
func main() {
	...
	b := botControl.NewControlBot(botAPI, db)
	databases.CsvToPostgres("tasks.csv", b.DB)
	b.Chapters = databases.CountChapters(b.DB)
	databases.CreateUsers(b.DB)
	databases.CreateNotes(b.DB)
